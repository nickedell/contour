	...
	<div className="min-h-screen bg-white dark:bg-[#121417] text-black dark:text-white px-6 py-8">
	  ...
	  {loading ? (
		...
		<div className="mt-6 grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
		  {rows.map((s) => (
			<Link key={s.id} href={`/personas/set/${s.id}`} className="rounded-xl border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-950 p-4 hover:shadow">
			  ...
			  <div className="mt-3 text-xs text-neutral-500">{s.persona_count || 0} persona{s.persona_count === 1 ? '' : 's'}</div>
			</Link>
		  ))}
		</div>
	  )}
